...
func ArrayColumnN[T, V any](array []T, k any) any {
...
resultArr := ArrayColumnN[struct_name, string](list, "Name")
resultArr.([]string) // 获取到的数据表可以直接断言成泛型列表传入的值类型使用

// 二维数组调用方式
// [[]int, string]  类型分别对应 [第二维数组的类型, 获取的值的类型]
resultArr := arrays.ArrayColumnN[[]int, int](list, 0)
resultArr.([]int) // 获取到的数据表可以直接断言成泛型列表传入的值类型使用

// 切片Map调用方式
// [map[string]interface{}, int]  类型分别对应 [数组下map的具体类型, 获取的值的类型]
resultArr := arrays.ArrayColumnN[map[string]interface{}, int](list, "id"))
resultArr.([]int) // 获取到的数据表可以直接断言成泛型列表传入的值类型使用
*/

func ArrayColumnD[T, V any](array []*T, k any) any {
	values := make([]V, len(array))

	for j, v := range array {
		t := reflect.TypeOf(v).Elem()
		immutable := reflect.ValueOf(v).Elem() // value

		for i := 0; i < t.NumField(); i++ {
			if t.Field(i).Name == k {
				//fmt.Printf("info: %s, doc: %s\n", t.Field(i).Tag.Get("info"), t.Field(i).Tag.Get("doc"))
				fieldValue := immutable.Field(i) // field vlaue
				values[j] = fieldValue.Interface().(V)
			}
		}

		//immutable := reflect.ValueOf(v).Elem()
		//values[j] = immutable.FieldByName(reflect.ValueOf(k).String()).Interface().(V)
	}
...
/*
func ColumnAny[T any](slice []T, column string) []interface{} {
	s := make([]interface{}, 0)
